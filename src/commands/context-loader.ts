...
const MCP_NOT_CONFIGURED =
  "Context-loader MCP is not configured. Run: kweaverc context-loader config set --kn-id <kn-id>";
...
function ensureContextLoaderConfig(): {
  ...
  if (!platform) {
    throw new Error("No platform selected. Run: kweaverc auth <platform-url>");
  }
...
export async function runContextLoaderCommand(args: string[]): Promise<number> {
  ...
  if (!subcommand || subcommand === "--help" || subcommand === "-h") {
    console.log(`kweaverc context-loader
...
Examples:
  kweaverc context-loader config set --kn-id d5iv6c9818p72mpje8pg
  kweaverc context-loader config set --kn-id xyz123 --name project-a
...
async function runConfigCommand(args: string[]): Promise<number> {
  ...
  if (!action || action === "--help" || action === "-h") {
    console.log(`kweaverc context-loader config
...
Subcommands:
  ...
  const platform = getCurrentPlatform();
  if (!platform) {
    console.error("No platform selected. Run: kweaverc auth <platform-url>");
    return 1;
  }
  ...
  if (action === "set") {
    ...
    if (!knId) {
      console.error("Usage: kweaverc context-loader config set --kn-id <id> [--name <name>]");
      return 1;
    }
  ...
  if (action === "use") {
    const name = rest[0];
    if (!name) {
      console.error("Usage: kweaverc context-loader config use <name>");
      return 1;
    }
    try {
  ...
  if (action === "remove") {
    const name = rest[0];
    if (!name) {
      console.error("Usage: kweaverc context-loader config remove <name>");
      return 1;
    }
...
async function runKnSearch(
  ...
  if (!query) {
    console.error("Usage: kweaverc context-loader kn-search <query> [--only-schema]");
    return 1;
  }
...
async function runKnSchemaSearch(
  ...
  if (!query) {
    console.error("Usage: kweaverc context-loader kn-schema-search <query> [--max N]");
    return 1;
  }
