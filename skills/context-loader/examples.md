...
以下每一步都可在终端用 kweaverc 执行；JSON 参数建议用单引号包裹，避免 shell 转义。若你还没确定要查哪个知识网络，先执行 `kweaverc bkn list --pretty`，必要时再用 `kweaverc bkn get <kn-id> --stats --pretty` 或 `kweaverc bkn export <kn-id> --pretty` 确认。可选先列出可用工具：`kweaverc context-loader tools --pretty`。
...
| Step 3 | `kweaverc context-loader query-instance-subgraph '{"relation_type_paths":[{"object_types":[{"id":"disease","condition":{...},"limit":1},{"id":"drug","condition":{"operation":"and","sub_conditions":[]},"limit":10}],"relation_types":[{"relation_type_id":"treats","source_object_type_id":"drug","target_object_type_id":"disease"}]}]}' --pretty`（省略号处填完整 condition） |
| Step 4 | `kweaverc context-loader get-logic-properties '{"ot_id":"drug","query":"高血压治疗药品的月销量","_instance_identities":[{"drug_id":"DRUG_001"},{"drug_id":"DRUG_002"}],"properties":["monthly_sales"]}' --pretty` |
| Step 5 | `kweaverc context-loader get-action-info '{"at_id":"prescribe","_instance_identity":{"drug_id":"DRUG_001"}}' --pretty` |

---

## Step 1：kn_search（Layer 1 - 探索发现）

**调用**（MCP） / **CLI 等价**：`kweaverc context-loader kn-search "高血压 治疗 药品" --only-schema [--pretty]`
...
**调用**（MCP） / **CLI 等价**：`kweaverc context-loader query-object-instance '{"ot_id":"disease","limit":5,"condition":{...}}' --pretty`（将下方 JSON 压成单行放入单引号内）
...
**调用**（MCP） / **CLI 等价**：将下方 `relation_type_paths` 的 JSON 压成单行，执行 `kweaverc context-loader query-instance-subgraph '<json>' --pretty`
...
**调用**（MCP） / **CLI 等价**：`kweaverc context-loader get-logic-properties '<下方 JSON 单行>' --pretty`
...
**调用**（MCP） / **CLI 等价**：`kweaverc context-loader get-action-info '{"at_id":"prescribe","_instance_identity":{...}}' --pretty`
...
`value_from` 仅支持 `"const"`，**必须**与 `value` 同时出现。在 kweaverc 中，整个 condition 作为 `query-object-instance` 或 `query-instance-subgraph` 的 JSON 参数的一部分传入。
